...
set_scale="0" #set scale for compilation
...
for sample in MinimumBias #SingleMu #SingleElectron JetHT MinimumBias MET Tau SinglePhoton DoubleElectron
  do
  ...
  if [ -f *"$sample"*"$rel_old"* ] && [ -f *"$sample"*"$rel_new"* ] ; then
      ...
      release=CMSSW_"$rel_new"_"$sample"_Run_"$run"frozenHLT_unscaled
